...
class RedisCacheSpec extends WriteableCacheSpec {
    ...
    Cache getSubject() {
        ...
        return new RedisCache('test', new JedisClientDelegate(source), mapper, RedisCacheOptions.builder().maxMset(MAX_MSET_SIZE).maxMergeBatch(MAX_MERGE_COUNT).build(), cacheMetrics)
    }
    ...
    def 'should merge #mergeCount items at a time'() {
        ...
        def cache = new RedisCache(
            'test',
            new JedisClientDelegate(new JedisPoolSource(pool)),
            new ObjectMapper(),
