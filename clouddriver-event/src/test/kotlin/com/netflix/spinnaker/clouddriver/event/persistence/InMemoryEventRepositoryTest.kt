...
class InMemoryEventRepositoryTest : JUnit5Minutests {
  ...
  fun tests() = rootContext<Fixture> {
    ...
    test("save appends aggregate events") {
      ...
      subject.save(event.getMetadata().aggregateType, event.getMetadata().aggregateId, 0L, listOf(event))
      ...
      subject.save(event2.getMetadata().aggregateType, event2.getMetadata().aggregateId, 1L, listOf(event2))
    ...
    test("saving with a new aggregate with a non-zero originating version fails") {
      ...
      assertThrows<AggregateChangeRejectedException> {
        subject.save(event.getMetadata().aggregateType, event.getMetadata().aggregateId, 10L, listOf(event))
      }
    }
    ...
    context("listing aggregates") {
      ...
      test("not providing a type") {
        listOf(event1, event2, event3).forEach {
          subject.save(it.getMetadata().aggregateType, it.getMetadata().aggregateId, 0L, listOf(it))
        }
