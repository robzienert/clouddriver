...
import com.netflix.spinnaker.clouddriver.event.ComposedSpinnakerEvent
...
class SqlEventRepository(
      ...
      withPool(POOL_NAME) {
        jooq.transactional(retryRegistry.retry("eventSave", retry)) { ctx ->
          ...
          val aggregate = ctx.maybeGetAggregate(aggregateCondition) ?: {
            ...
            // Aggregate tokens are globally unique across all aggregates and calculated monotonically.
            val initialAggregate = mapOf(
              field("aggregate_type") to aggregateType,
              field("aggregate_id") to aggregateId,
              field("token") to getNextMonotonicToken(ctx),
              field("version") to 0
            )

            // Ensure the aggregate exists before proceeding
          ...
          // TODO(rz): This query isn't needed. We can `select coalesce(max(sequence), 0) + 1 where ...` in the insert
          // statement. Probably?
          ...
          // Update the event table with a new version
  ...
  private fun SpinnakerEvent.initialize(
    ...
    if (this is ComposedSpinnakerEvent) {
  ...
  override fun listAggregates(criteria: ListAggregatesCriteria): EventRepository.ListAggregatesResponse {
    ...
    return withPool(POOL_NAME) {
      ...
      EventRepository.ListAggregatesResponse(
  ...
  private fun getNextMonotonicToken(ctx: DSLContext): String {
    val attempts = 10
    (10 downTo 1).forEachIndexed { i, _ ->
      val current = ctx.select(field("token"))
        .from(AGGREGATES_TABLE)
        .orderBy(field("token").desc())
        .limit(1)
        .fetchOne(0, String::class.java)

      val token = current
        ?.let { ULID.parseULID(it) }
        ?.let { ulid.nextStrictlyMonotonicValue(it) }
        ?: Optional.of(ulid.nextValue())

      if (token.isPresent) {
        log.trace("Calculated next monotonic token after ${11 - i} attempts")
        return token.get().toString()
      }
    }
    throw SqlEventSystemException("Unable to calculate next monotonic token after $attempts attempts")
  }
