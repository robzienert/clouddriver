...
public class TitusServiceJobPredicate implements Predicate<Saga> {
  ...
  public boolean test(Saga saga) {
    return saga.getEvents().stream()
        .filter(
            e -> PrepareTitusDeploy.PrepareTitusDeployCommand.class.isAssignableFrom(e.getClass()))
        .findFirst()
        .map(
            e -> {
              String jobType =
                  ((PrepareTitusDeploy.PrepareTitusDeployCommand) e).getDescription().getJobType();
