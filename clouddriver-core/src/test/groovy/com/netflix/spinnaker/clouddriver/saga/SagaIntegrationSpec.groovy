...
class SagaIntegrationSpec extends Specification {
  ...
  def "basic api"() {
    given:
    SagaProcessor sagaProcessor = new DefaultSagaProcessor(new MemorySagaRepository(), new NoopRegistry(), Optional.empty())

    and:
    Saga saga = new SagaKatoBridgeDsl(new DefaultTask())
      .inputs([:])
