  ...
  fun <T> applyBlocking(flow: SagaFlow, startingCommand: SagaCommand): T? {
    ...
    SagaFlowIterator(sagaRepository, applicationContext, initialSaga, flow).forEach { flowState ->
      ...
      if (!saga.completed(requiredCommand)) {
        ...
        val newEvents: MutableList<SagaEvent> = mutableListOf(
          SagaCommandCompleted(saga, getStepCommandName(stepCommand))
        )
        result.nextCommand?.let {
          // TODO(rz): Would be nice to flag commands that are optional so its clearer in the event log
          if (it is ManyCommands) {
            newEvents.addAll(it.commands)
          } else {
            newEvents.add(it)
          }
        }
